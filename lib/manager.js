...
internals.runner = function (worker, destroyMessageFun, retryMessageFun, notifiers) {
  return function (job, callback) {
    notifiers.progress('started processing job', job.data);

    worker(job.data, function (err) {
      if(err && err.retryable) {
        return retryMessageFun(job.handle, callback);
      } else if(err) {
...
module.exports = function (queue, worker, options) {
  ...
  var retryMessageFun = _.partial(queue.retry, options.retryInterval);

  var runner = internals.runner(worker, queue.destroy, retryMessageFun, settings.notifiers);
